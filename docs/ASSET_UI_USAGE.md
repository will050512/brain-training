# 素材優先順序與 UI 使用位置 / Asset Priority & UI Usage

本文件整理「實際 UI 使用位置」與「素材優先補齊順序」，方便設計/製作交付。
以長者友善為優先：高對比、清楚辨識、避免過度細節。

This document maps UI usage and asset priority for delivery. Elder-friendly UX first: high contrast, clear shapes, avoid excessive detail.

## 優先順序（P0 → P2）/ Priority

### P0（核心體驗，請優先補齊）/ P0 (Core)
...
- `ASSET_GUIDE.md`：實際使用素材路徑（根目錄）
- `docs/ASSET_REQUIREMENTS.md`：交付規格

- `public/assets_manifest.json`: game icon list (keep in sync).
- `ASSET_GUIDE.md`: actual asset usage paths (repo root).
- `docs/ASSET_REQUIREMENTS.md`: delivery specs.
