...
- 完成並提交 `docs/DEVELOPER_GUIDE.md` 的變更（已完成）
- 選擇是否同步修改 `package.json` 新增 script 與加入 ESLint/Prettier 設定（需你確認是否要我一併修改）

需要我現在替你：1) 直接在 repo 中新增 `type-check` 與 `lint` scripts 並建立基本 ESLint 設定，或 2) 僅提交文件更新並等待你的回覆？
